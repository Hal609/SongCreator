...
def countCurrentSpectrograms():
    ...
    for inst in counts.keys():
        counts[inst] = len([name for name in os.listdir(f'./dataset_root/{inst}')])
        print(f"{inst}:", counts[inst])
        
    return counts

def createNSingleInstSpectrograms(n, keepWAVs = False):
    global kick, snare, hihat, ride

    counts = countCurrentSpectrograms() 
    ...
    for i in range(n):
        ...
        filepath = path.dirname(__file__) + f"/../temp/{inst}_{voice}_{counts[inst]:04}.wav"
        # print(f"Exporting wav to {filepath}.")
        fullBeat.export(filepath)
        # print("Export finished")

        gloPath = pathlib.Path(__file__).parent.resolve()
        wavToSpect(filepath, f"{gloPath}/../dataset_root/{inst}/{inst}_{counts[inst]:04}.bmp")
