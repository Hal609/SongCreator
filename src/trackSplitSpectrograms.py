...
def countCurrentSpectrograms():       
    return len([name for name in os.listdir(f'./splitTrackData')])
    ...
    for i in range(n):
        ...
        for k in range(num_instruments):
            ...
            if "melody" in selected_items:
                ...
                instrumentTracks[inst] = instrumentTracks[inst].overlay(addMelody(fullMelodyNotes, instSamplePath, silence))
            if "chords" in selected_items:
                fullBeat = fullBeat.overlay(addChords(key, chordProgression, instSamplePath, silence))
                instrumentTracks[inst] = instrumentTracks[inst].overlay(addChords(key, chordProgression, instSamplePath, silence))
            if "bass" in selected_items:
                fullBeat = fullBeat.overlay(addMelody(fullBassNotes, instSamplePath, silence))
                instrumentTracks[inst] = instrumentTracks[inst].overlay(addMelody(fullBassNotes, instSamplePath, silence))
            if "drums" in selected_items:
                for beat in tracks:
                    fullBeat = fullBeat.overlay(addTrack(beat, silence))
                    instrumentTracks[inst] = instrumentTracks[inst].overlay(addTrack(beat, silence))
                    
            filepath = path.dirname(__file__) + f"/../temp/track{count:04}_{inst}.wav"
            trackPaths[inst] = filepath
            instrumentTracks[inst].export(filepath)
